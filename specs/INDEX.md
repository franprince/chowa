...
| 2026-08-02 | [quota-resume-orchestrator](2026-08-02-quota-resume-orchestrator/spec.md) | Draft | A Claude Code Skill can't act after its own session is quota-blocked (abrupt cutoff, read-only `StopFailure` hook, session-scoped scheduled tasks) — only an external wrapper can. Scopes down `feat/quota-limit-detection`'s tangled subprocess-handoff idea to just the proven, safe piece: a task queue that dispatches via `claude -p --resume --permission-mode dontAsk` (no `--dangerously-skip-permissions`), detects real completion via the structured `result.is_error` field, and correlates failures against a zero-token `get_usage` quota probe before scheduling an auto-resume — subscription-only, no API keys. |
