...
| 2026-08-04 | [cross-repo-skill-source-of-truth](2026-08-04-cross-repo-skill-source-of-truth/spec.md) | Draft | `franprince/skills-marketplace`'s `chowa-skill` was a one-time hand-adapted copy of this repo's own skill, and already drifted (this repo gained a PR closing-line convention `chowa-skill` never got). Proposes inverting the relationship: `chowa-skill` becomes the canonical source for shared workflow content, and chowa's three skill files are generated from it plus a chowa-only overlay (CLI invocation, model routing, session auto-resume). Sync mechanism (submodule vs. pinned network fetch vs. manual) is the open crux decision. |
