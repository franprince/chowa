 ...
 *
 * Chōwa's branching rule — never push directly to `main`/`master`, outside
 * a `release/*` or `hotfix/*` PR — was prose in the skill, which a model may
 * or may not act on. This makes it mechanical.
 *
 ...
 * 3. **No dependencies.** Plain Node, no jq, no packages, so it runs
 *    wherever the bundled engine does.
 */
...
const GUIDANCE =
  'Chōwa: never push directly to main/master. Branch (feat/*, fix/*, docs/*, ' +
...
// non-ASCII character in the install path (Chōwa's own repo directory has
// one) is percent-encoded in `import.meta.url` but not in `argv[1]`, so the
// naive string comparison silently never matches and the hook does nothing.
// `realpathSync` additionally covers being invoked through a symlink, where
// the two would otherwise disagree. Failure here is silent by nature, so it
