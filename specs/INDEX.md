...
| 2026-08-04 | [cross-repo-skill-source-of-truth](2026-08-04-cross-repo-skill-source-of-truth/spec.md) | Approved | `franprince/chowa-skill` was a one-time hand-adapted copy of this repo's own skill, and already drifted (this repo gained a PR closing-line convention `chowa-skill` never got). A new template in `chowa-skill`'s own repo, tagging every paragraph `shared`/`chowa-only`/`chowa-skill-only`, becomes the real source of truth — both `chowa-skill` and chowa's own three skill files become generated outputs of it, chowa's fetched via a pinned commit SHA (deterministic CI, no submodule onboarding tax). |
| 2026-08-06 | [reverse-engineering-skill](2026-08-06-reverse-engineering-skill/spec.md) | Draft | Dedicated pre-planning investigation skill (`chowa-reverse-engineer`) to systematically audit, reverse engineer, and extract architecture profiles from unfamiliar codebases, outputting structured context consumable by Chōwa's Stage 1 (`spec.md`) and Stage 2 (`implementation_plan.md`) feature planning. |
