...
| 2026-08-02 | [mechanical-task-model-delegation](2026-08-02-mechanical-task-model-delegation/spec.md) | Draft | `chowa.config.ts`'s router only governs LLM calls Chōwa's own CLI makes directly (commit messages, PR descriptions) — it has no bearing on the model running the live Claude Code agent that executes the pipeline itself. Explores whether mechanical sub-tasks performed inline by the live agent should delegate to a cheap-model subagent, since Claude Code's per-skill `model:` frontmatter is turn-scoped and doesn't fit a multi-turn, mixed-task-kind pipeline. |
