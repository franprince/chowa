...
var NEVER = INVALID;
// src/ledger/store.ts
var ledgerEntrySchema = exports_external.object({
...
// src/integrations/claude-code/quotaProbe.ts
import { spawn as spawn2 } from "node:child_process";
var CONTROL_REQUEST = `${JSON.stringify({
  type: "control_request",
  request_id: "chowa_probe",
  request: { subtype: "get_usage" }
})}
`;
var RATE_LIMIT_KEYS = ["five_hour", "seven_day"];
async function probeUsage(options = {}) {
...
function runProbe(binary, timeoutMs) {
  return new Promise((resolve, reject) => {
    const child = spawn2(binary, ["-p", "--verbose", "--input-format", "stream-json", "--output-format", "stream-json"], { stdio: ["pipe", "pipe", "pipe"] });
    let out = "";
    let err = "";
...
function extractUsageResponse(stdout) {
...
`)) {
    ...
    if (!isRecord(parsed) || parsed.type !== "control_response")
      continue;
...
function parseSnapshot(payload) {
  ...
  if (windows.size === 0) {
    throw new Error("get_usage returned no parseable rate-limit windows — the experimental response shape may have changed.");
  }
  return {
...
// src/integrations/claude-code/hooks/hookRunner.ts
...
function runHookMain(hookName, fn) {
  fn().catch((error) => {
...
// src/integrations/claude-code/hooks/stopFailure.ts
function tightestWindow(snapshot) {
  let tightest = null;
  for (const [name, window2] of snapshot.windows) {
    if (!tightest || window2.utilization > tightest.window.utilization) {
      tightest = { name, window: window2 };
    }
  }
  return tightest;
}
async function main(payload, deps = {}) {
  ...
  const updated = stampQuota(ledger, key, tightest.name, tightest.window.resetsAt.toISOString(), undefined, payload.last_assistant_message);
