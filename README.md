...
- `plugins/chowa/skills/chowa/SKILL.md` is the **canonical** skill. `.agents/skills/chowa/SKILL.md` is generated from it by `bun run sync:skill`; don't edit it by hand — CI fails if the two drift. Claude-Code-only mechanisms (the `chowa-mechanical` subagent, for one) are stripped from the portable copy during the sync. `.claude/skills/chowa/SKILL.md` is project-local to this repo and governs work here only.
